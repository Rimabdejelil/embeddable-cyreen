...
export default (props: Props) => {
  const { columns: originalColumns, results } = props;
  ...
  const getUniqueStatuses = (data: any[]) => {
    ...
    data.forEach(row => {
      const status = row['mon_heartbeat.status'];
      if (status) {
  ...
  const columns = [
    ...originalColumns.filter(col => col.name !== 'mon_heartbeat.status'),
    ...statusColumns
  ];
                    ...
                    {columns.map((column, columnIndex) => (
                      <th
                        ...
                        style={{
                          ...
                          left: columnIndex === 0 ? '-1px' : undefined,
                  ...
                  {results?.data?.map((row, rowIndex) => (
                    ...
                    >
                      {columns.map((column, columnIndex) => {
                        ...
                        const isCurrentStatus = row['mon_heartbeat.status']?.toLowerCase() === statusType;
                            ...
                            {isCurrentStatus ? (
                              <div style={{
                                ...
                                <div style={{
                                  ...
                                  height: '12px',
                                  borderRadius: '50%',
                                  backgroundColor: getStatusColor(row['mon_heartbeat.status'])
                                }} />
                              </div>
                            ) : null}
                          </td>
